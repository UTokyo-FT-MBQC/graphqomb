...
- **MPP Rewriter Segment Fallback**: `rewrite_to_mpp()` accepts `fallback="segment"` (default `"circuit"` keeps the current behavior). Instead of abandoning the whole circuit when one segment cannot be represented or verified, only the failing segments are emitted gate-level and the rest become `MPP` form, so a mostly-standard circuit with a few irregular rounds still imports as a mostly-foliated pattern. In this mode, segments that reuse a measurement post-state without a reset or apply measurement-record feedback also stay gate-level instead of raising; noise remains rejected. Because a rewritten segment is only verified with its measured-out post-states reset, a segment that consumes such a post-state forces the producing segment to stay gate-level too (verbatim segments preserve post-states exactly), rerunning the rewrite until that fixed point is reached. A deterministic-minus record is never padded to `MPAD 1` in this mode (the Stim importer rejects it); the unsubstituted retry keeps it a real signed measurement, so records consumed by verbatim feedback keep their parity. The new `MppRewriteResult.fallback_segments` lists the gate-level segments in every mode. On Gidney's logical-Y circuits, where only the Y-transition rounds fail to rewrite, the composed rewrite-and-import path shrinks the graph well past safe TICK merging (d=9, r=9: 9951 nodes direct, 8217 with `merge_safe_ticks`, 6237 with segment fallback; all verified by detector counts and an empty noiseless DEM).
- **Stim Import Safe TICK Merging**: `stim_circuit_to_pattern()`, `stim_text_to_pattern()`, and `stim_file_to_pattern()` accept `merge_safe_ticks` (default off). TICK boundaries are optimization barriers, so a finely ticked circuit imports to a larger graph than the same instructions in fewer blocks; the option drops every TICK whose removal cannot change the imported semantics before analysis, letting gate runs cancel across former boundaries and single-qubit Cliffords fold into adjacent resets and measurements. Removal never reorders instructions, and block normalization preserves per-qubit instruction order and measurement-record order, so the only TICKs kept are those separating two blocks that both contain Pauli-product measurements, which preserves per-block `StimMppExtraction` grouping and block-level commutation validation. On Gidney's gate-level logical-Y surface-code circuits this shrinks the imported graph by 17-19% (for example d=9, r=3: 7167 to 5913 nodes) and speeds up import.
