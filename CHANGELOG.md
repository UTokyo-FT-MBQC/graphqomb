...
- **Exact Pending-Clifford MPP Rewrite**: `rewrite_to_mpp` now uses the instrument identity `Π(P) U = U Π(U† P U)` directly: Clifford bodies move unchanged behind their pulled Pauli measurements and materialize at resets, record-feedback barriers, or circuit exit. A matching reset stabilizer on the directly measured source qubit is removed to expose data-only MPP checks. At a measure-reset boundary, equal canonical flows between the local reset/body/measurement channel and the reduced MPP/reset channel certify that Foliation has replaced the source extraction ancilla, allowing its redundant pending body to be discarded; a noncontractible data Clifford remains unchanged behind the MPP. The exactly equivalent `MppRewriteResult.circuit` retains independent reset outputs, while `foliation_circuit` removes reset-only source ancillas superseded by Foliation and `eliminated_qubits` lists them. This replaces residual-frame synthesis, retries, post-state reuse tracking, and circuit/segment fallback with one deterministic path that preserves the complete retained-qubit channel and supports variable-length `SPP`/`SPP_DAG`. Distinct commuting products inside one source `TICK` interval share one imported MPP graph layer, while source `TICK`s remain hard layer boundaries; removing a contracted measure-reset also preserves a hard boundary before the next MPP, and a repeated identical support starts a new internal layer, so identical checks cannot fuse together. The historical `fallback` argument remains a compatibility alias and `fallback_segments` is always empty.
