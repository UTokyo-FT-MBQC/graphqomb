...
- **Experimental Stim MPP Rewriter**: Added `graphqomb.stim_mpp_rewriter.rewrite_to_mpp` to rewrite noiseless gate-level syndrome-extraction circuits into abstract `MPP` measurements. Each ancilla measurement Pauli is conjugated backwards through its segment's Clifford body, initialized-ancilla stabilizers are substituted by `+1`, and the inferred data product replaces the measurement at the same record position, so `DETECTOR` and `OBSERVABLE_INCLUDE` annotations are copied verbatim. `MR` splits into `MPP` plus a reset, a measurement following a post-measurement reset (including the implicit reset of `MR`) starts a new segment so trailing data measurements pass through verbatim, valid repeated-qubit `MPP` factors are reduced in the sidecar mapping, inferred `+I`/`-I` products become `MPAD 0`/`MPAD 1`, residual Clifford frames on surviving qubits are preserved, and a `CheckMapping` sidecar links every Pauli-measurement record to its inferred product. By default each rewritten segment is verified against its source by cross-checking stabilizer-flow generators (with measured-out ancillas reset in both copies), using matching canonical generators as a fast path. If initialized-ancilla substitution or residual-frame synthesis cannot preserve a segment, the rewriter retries exact pulled-back products and finally falls back to the original gate-level circuit with reset lifetimes assigned fresh Stim qubit ids. Noise, classical feedback, and reuse of unreset measurement post-states raise `UnsupportedSyndromeCircuitError`.
