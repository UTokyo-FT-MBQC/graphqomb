...
- **Initialization Tags**: Stim reset instruction tags (`R[tag]`, `RX[tag]`, `RY[tag]`) survive the import/export round trip, mirroring detector tags. `GraphState.register_input()` accepts `init_tag`, `Pattern` gains `input_initialization_tags`, and `stim_compile` re-emits each input's tag on its reset instruction. The `.ptn` format serializes them as `.input_tag[tag] node` lines under new format version 4.
