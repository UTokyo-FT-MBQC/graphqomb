...
- **Initialization Tags**: Stim instruction tags on reset instructions (`R[tag]`, `RX[tag]`, `RY[tag]`) now survive the import/export round trip, mirroring detector tags. `GraphState.register_input()` accepts `init_tag` and stores it on the new `input_initialization_tags` property (carried through `compose()`, `from_graph()`, `from_base_graph_state()`, and local-Clifford expansion); `qompile` copies it onto the new `Pattern.input_initialization_tags` field. The Stim importer fills the tags from each wire's last leading reset — including mid-circuit resets split onto fresh wires and the re-preparation half of measure-reset gates (`MR[tag]`), while plain measurement reuse stays untagged — and `stim_compile` re-emits each input's tag on its reset instruction, escaped with Stim's tag escape language. The `.ptn` format serializes tagged inputs as `.input_tag[tag] node` lines and introduces format version 4 for them; tag-free files keep their smaller version header as before.
