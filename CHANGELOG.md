...
- **Detector Determinism with Z Measurements and Z Initialization**: `PauliFrame.detector_determinism()` treated every Z-measured node as if it owned a single-qubit Z stabilizer, unconditionally erasing its Z support from neighboring graph stabilizers and matching its own Z measurement for free. A detector whose group contained a Z-measured node prepared in a non-Z basis was therefore reported deterministic even when its parity is physically random (for example a lone Z measurement on a plus-state node). Conversely, Z-initialized nodes — whose single-qubit Z stabilizer really does make Z support on them freely absorbable — were compared exactly, flagging genuinely deterministic detectors as non-deterministic whenever a group member neighbors a Z-initialized node that is not Z-measured. `detector_stabilizers()` now drops Z-initialized nodes (instead of Z-measured ones) from neighbor supports and only credits a Z-measured node with its own Z stabilizer when that node is Z-initialized, so a Z measurement on any other node must draw its Z support from the graph stabilizers of its neighbors. The reported verdicts now agree with Stim's determinism analysis of the compiled circuit across every Pauli initialization/measurement combination on the checked graphs, and the returned detector stabilizers are always genuine elements of the resource-state stabilizer group.
