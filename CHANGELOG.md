...
- **Compile-input pruning**: The new `graphqomb.pruning` module offers `prune_z_nodes`, which removes Z-prepared inputs and Z-measured nodes from a graph state and drops them from the correction flows, parity check groups, and logical observables in one pass, and `prune_isolated_components`, which deletes connected components touching neither an output node nor a logical observable seed (e.g. fragments left behind by Z pruning). Both return a `PruneResult` that preserves node indices and feeds directly into `qompile`.
